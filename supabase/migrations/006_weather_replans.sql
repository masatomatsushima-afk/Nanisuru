-- 006: weather_replans (weather-based plan adjustments)
-- App: src/lib/weather-replans.ts
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'weather_replans'
      AND policyname = 'weather_replans_read_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'weather_replans'
      AND policyname = 'weather_replans_insert_own'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'weather_replans'
      AND policyname = 'weather_replans_delete_own'
  ) THEN
    CREATE POLICY "weather_replans_delete_own" ON public.weather_replans
